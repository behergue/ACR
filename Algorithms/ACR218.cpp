...
bintree<int> post(stack<int> & postorder, vector<int> const & inorder) {
	...
	if (inorder.empty()) {
		...
		int raiz = postorder.top();
		bool encontrado = false;
		int i = 0;

		while (!encontrado && i < inorder.size()) {
			if (inorder[i] == raiz) {
				encontrado = true;
			}
			i++;
		...
		auto ibegin = inorder.begin();
		auto iend = inorder.end();

		vector<int> left(ibegin, ibegin + pos);
		vector<int> right(ibegin + pos + 1, iend);

		postorder.pop();

		bintree<int> arbolRight = post(postorder, right);
		bintree<int> arbolLeft = post(postorder, left);
		
		return { arbolLeft, raiz, arbolRight };
	}
}
...
bool resuelveCaso() {
	...
	for (int i = 0; i < num; i++) {
		cin >> valor;
		postorder.push(valor);
	}

	bintree<int> arbol = post(postorder, inorder);
	vector<int> preorder = arbol.preorder();
