...
bintree<int> post(queue<int> & preorder, vector<int> const & inorder) {
	...
	if (inorder.empty()) {
		...
		bintree<int> arbolLeft = post(preorder, left);
		bintree<int> arbolRight = post(preorder, right);
...
bool resuelveCaso() {
	...
	bintree<int> arbol = post(preorder, inorder);
	vector<int> postorder = arbol.postorder();
